...
class OrderController extends AppControllerAdmin {
...
FROM `order` JOIN `user` on `order`.`user_id` = `user`.`id` JOIN `order_product` ON `order`.`id` = `order_product`.`order_id`
...
GROUP BY `order`.`id` ORDER BY `order`.`status`, `order`.`id` LIMIT 1", [$order_id]);
        ...
        $this->set(compact('order', 'order_product'));
    }
    ...
    public function deleteAction() {
        ...
        $order = \R::load('order', $order_id);
        \R::trash($order);
        $_SESSION['success'] = 'Заказ уася удален!';
